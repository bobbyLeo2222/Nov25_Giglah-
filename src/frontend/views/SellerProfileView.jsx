...
function SellerProfileView({
  ...
  const aboutText = selectedSeller.about || selectedSeller.bio || ''
  const hasAbout =
    Boolean(aboutText) &&
    String(aboutText).trim() !== String(selectedSeller.headline || '').trim()
        ...
        <div className="space-y-4">
            {showBioCard && (
              <div className="flex items-start gap-4 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-4">
                <UserAvatar
                  ...
                  fallbackClassName="h-16 w-16 rounded-2xl bg-white text-slate-500"
            ...
            {(hasSpecialties || hasLanguages) && (
              <div className="rounded-2xl border border-slate-100 bg-white px-4 py-4 shadow-sm">
                ...
                {hasLanguages && (
                  <div className={hasSpecialties ? 'mt-4' : ''}>
