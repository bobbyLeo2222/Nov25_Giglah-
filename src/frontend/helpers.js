...
const MODE_STORAGE_KEY = 'giglah_mode'

const getStoredUserMode = () => {
  if (typeof localStorage === 'undefined') return ''
  return localStorage.getItem(MODE_STORAGE_KEY) || ''
}

const setStoredUserMode = (mode) => {
  if (typeof localStorage === 'undefined') return
  if (mode) {
...
const mapUserFromApi = (apiUser) => {
  ...
  if (apiUser.role === 'seller' && storedMode) {
    return { ...apiUser, isSeller: storedMode === 'seller' }
  }
  return { ...apiUser, isSeller: apiUser.role === 'seller' }
}
