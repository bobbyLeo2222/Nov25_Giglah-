...
import OrderHistoryView from '@/frontend/views/OrderHistoryView'
...
import { mergeSellerAndBuyerOrders, normalizeOrderStatus } from '@/frontend/orderUtils'
import {
  buildSellerId,
  formatFileSize,
  mapUserFromApi,
...
const DEFAULT_ACTION_COOLDOWN_MS = 900
const MUTATION_ACTION_COOLDOWN_MS = 1400
const MAX_NOTIFICATIONS = 30
const ACTIVE_ORDER_STATUSES = new Set(['pending', 'in_progress', 'awaiting_completion'])
const GOOGLE_CLIENT_ID =
  import.meta.env.VITE_GOOGLE_CLIENT_ID || import.meta.env.REACT_APP_GOOGLE_CLIENT_ID || ''
...
const getChatMessageIdentity = (message) => String(message?._id || message?.id || '')
...
const sortChatMessages = (messages = []) =>
  [...messages].sort((left, right) => {
    ...
    return getChatMessageIdentity(left).localeCompare(getChatMessageIdentity(right))
  })

const mergeChatMessages = (messages = [], optimisticMessages = []) => {
  const merged = new Map()

  messages.forEach((message) => {
    const key = getChatMessageIdentity(message)
    if (!key) return
...
const upsertOrderList = (orders = [], nextOrder) => {
  if (!nextOrder) return orders
  const nextId = getOrderIdentity(nextOrder)
  const dedupedOrders = nextId
    ? orders.filter((order) => getOrderIdentity(order) !== nextId)
    : [...orders]

  return [nextOrder, ...dedupedOrders].sort((left, right) => getOrderTimestamp(right) - getOrderTimestamp(left))
}

const getActiveOrderStatusMessage = (status) => {
  switch (normalizeOrderStatus(status)) {
    case 'pending':
      return 'A gig request for this chat is already pending.'
    case 'in_progress':
      return 'This gig is already in progress.'
    case 'awaiting_completion':
      return 'This gig is awaiting completion confirmation.'
    default:
      return 'An active order for this gig already exists.'
  }
}

const isActiveChatOrder = (order, { gigId = '', buyerId = '', sellerId = '' } = {}) => {
  if (!order || !gigId || !buyerId || !sellerId) return false
  if (String(order.gigId || '') !== String(gigId)) return false
  if (!ACTIVE_ORDER_STATUSES.has(normalizeOrderStatus(order.status))) return false

  const orderBuyerId = getComparableUserId(order.buyer)
  const orderSellerId = getComparableUserId(order.seller)
  return String(orderBuyerId || '') === String(buyerId) && String(orderSellerId || '') === String(sellerId)
}
...
const getOrderActivitySnapshot = (order) => ({
  ...
  buyerCancellationRequestedAt: order?.buyerCancellationRequestedAt
    ? new Date(order.buyerCancellationRequestedAt).getTime() || 0
    : 0,
  sellerCancellationRequestedAt: order?.sellerCancellationRequestedAt
    ? new Date(order.sellerCancellationRequestedAt).getTime() || 0
    : 0,
})
...
const getOrderEventKey = (previous, next) => {
  ...
  if (
    next.buyerCancellationRequestedAt !== previous.buyerCancellationRequestedAt ||
    next.sellerCancellationRequestedAt !== previous.sellerCancellationRequestedAt
  ) {
...
const getOrderNotificationContent = ({ order, eventKey, isBuyer, isSeller }) => {
  const gigTitle = order?.gigTitle || 'Your order'
  const action = { kind: isSeller ? 'seller-orders' : 'buyer-orders' }
  ...
  switch (eventKey) {
    case 'new':
      ...
      return {
        ...
        body: `${gigTitle} has been ordered by a buyer.`,
        action,
      }
    case 'accepted':
      return isSeller
    ...
    case 'cancellation_requested':
      return {
        ...
        body: `${gigTitle} - One party requested cancellation and is waiting for confirmation.`,
        action,
      }
...
const resolveViewFromPath = (pathname = '/') => {
  ...
  if (matchPath('/order-history/:orderId', pathname)) return 'order-history'
...
function App() {
  ...
  const [selectedOrderId, setSelectedOrderId] = useState(() => {
    if (typeof window === 'undefined') return ''
    return matchPath('/order-history/:orderId', window.location.pathname)?.params?.orderId || ''
  })
  ...
  const [pendingChatSendCount, setPendingChatSendCount] = useState(0)
  ...
  const optimisticChatMessagesRef = useRef({})
  const lastChatSendSignatureRef = useRef({ signature: '', at: 0 })
  const toastTimeoutsRef = useRef([])
  const actionCooldownsRef = useRef(new Map())
  const isSendingChatMessage = pendingChatSendCount > 0
  ...
  const runWithActionCooldown = useCallback(async (key, action, cooldownMs = DEFAULT_ACTION_COOLDOWN_MS) => {
    if (!key || typeof action !== 'function') return false
    const now = Date.now()
    const current = actionCooldownsRef.current.get(key)
    if (current && (current.pending || current.until > now)) return false

    actionCooldownsRef.current.set(key, { pending: true, until: now + cooldownMs })
    try {
      await action()
      return true
    } finally {
      actionCooldownsRef.current.set(key, {
        pending: false,
  ...
  const allKnownOrders = useMemo(
    () => mergeSellerAndBuyerOrders(sellerOrders, buyerOrders),
    [buyerOrders, sellerOrders],
  )

  const selectedOrder = useMemo(
    () => allKnownOrders.find((order) => String(order?._id || order?.id || '') === String(selectedOrderId || '')) || null,
  ...
  const selectedThreadRelatedOrders = useMemo(() => {
    if (!selectedThread) return null

    const threadOrderContext = {
      gigId: selectedThread.gigId || '',
      buyerId: selectedThread.buyerUserId || '',
      sellerId: selectedThread.sellerUserId || '',
    }

    const matchingOrders = [...buyerOrders, ...sellerOrders]
      .filter((order) => {
        if (!order || !threadOrderContext.gigId || !threadOrderContext.buyerId || !threadOrderContext.sellerId) {
          return false
        }
        if (String(order.gigId || '') !== String(threadOrderContext.gigId)) return false
        return (
          String(getComparableUserId(order.buyer) || '') === String(threadOrderContext.buyerId) &&
          String(getComparableUserId(order.seller) || '') === String(threadOrderContext.sellerId)
        )
      })
      .sort((left, right) => getOrderTimestamp(right) - getOrderTimestamp(left))

    return matchingOrders
  }, [buyerOrders, selectedThread, sellerOrders])

  const selectedThreadLatestOrder = useMemo(
    () => (selectedThreadRelatedOrders && selectedThreadRelatedOrders[0]) || null,
    [selectedThreadRelatedOrders],
  )

  const selectedThreadActiveOrder = useMemo(
    () =>
      (selectedThreadRelatedOrders || []).find((order) =>
        ACTIVE_ORDER_STATUSES.has(normalizeOrderStatus(order?.status)),
      ) || null,
    [selectedThreadRelatedOrders],
  )
  ...
  const pendingChatOrder = useMemo(() => {
    ...
    if (!selectedThreadActiveOrder) return null
    return normalizeOrderStatus(selectedThreadActiveOrder.status) === 'pending'
      ? selectedThreadActiveOrder
      : null
  }, [selectedThreadActiveOrder, user])

  const mergeThreadWithOptimisticMessages = useCallback((thread) => {
    if (!thread?.id) return thread
    const optimisticMessages = optimisticChatMessagesRef.current[thread.id] || []
    if (!optimisticMessages.length) return thread

    const mergedMessages = mergeChatMessages(thread.messages || [], optimisticMessages)
    const lastUpdatedAt = mergedMessages[mergedMessages.length - 1]?.sentAt || thread.lastUpdatedAt
    return { ...thread, messages: mergedMessages, lastUpdatedAt }
  }, [])

  const upsertChatThreadMessage = useCallback((threadId, nextMessage) => {
    if (!threadId || !nextMessage) return
    setChatThreads((prev) => {
      const nextThreads = prev.map((thread) => {
        if (thread.id !== threadId) return thread
        const mergedMessages = mergeChatMessages(thread.messages || [], [nextMessage])
        const lastUpdatedAt = mergedMessages[mergedMessages.length - 1]?.sentAt || thread.lastUpdatedAt
        return { ...thread, messages: mergedMessages, lastUpdatedAt }
      })
      return nextThreads.sort((left, right) => (right.lastUpdatedAt || 0) - (left.lastUpdatedAt || 0))
    })
  }, [])

  const setOptimisticMessageForThread = useCallback((threadId, nextMessage) => {
    if (!threadId || !nextMessage) return
    const current = optimisticChatMessagesRef.current
    optimisticChatMessagesRef.current = {
      ...current,
      [threadId]: [
        ...(current[threadId] || []).filter((message) => getChatMessageIdentity(message) !== getChatMessageIdentity(nextMessage)),
        nextMessage,
      ],
    }
    upsertChatThreadMessage(threadId, nextMessage)
  }, [upsertChatThreadMessage])

  const updateOptimisticMessageForThread = useCallback((threadId, messageId, updater) => {
    if (!threadId || !messageId || typeof updater !== 'function') return
    const current = optimisticChatMessagesRef.current
    const threadMessages = current[threadId] || []
    const nextThreadMessages = threadMessages.map((message) =>
      getChatMessageIdentity(message) === String(messageId) ? updater(message) : message,
    )
    optimisticChatMessagesRef.current = { ...current, [threadId]: nextThreadMessages }
    const updatedMessage = nextThreadMessages.find(
      (message) => getChatMessageIdentity(message) === String(messageId),
    )
    if (updatedMessage) {
      upsertChatThreadMessage(threadId, updatedMessage)
    }
  }, [upsertChatThreadMessage])

  const clearOptimisticMessageForThread = useCallback((threadId, messageId) => {
    if (!threadId || !messageId) return
    const current = optimisticChatMessagesRef.current
    const remainingMessages = (current[threadId] || []).filter(
      (message) => getChatMessageIdentity(message) !== String(messageId),
    )
    optimisticChatMessagesRef.current = { ...current, [threadId]: remainingMessages }
    setChatThreads((prev) => {
      const nextThreads = prev.map((thread) => {
        if (thread.id !== threadId) return thread
        ...
        const lastUpdatedAt = nextMessages[nextMessages.length - 1]?.sentAt || thread.lastUpdatedAt
        return { ...thread, messages: nextMessages, lastUpdatedAt }
      })
      return nextThreads.sort((left, right) => (right.lastUpdatedAt || 0) - (left.lastUpdatedAt || 0))
    })
  }, [])
  ...
  const syncViewFromPath = useCallback(() => {
    if (location.pathname === '/live-gigs') {
      const params = new URLSearchParams(location.search || '')
      const nextOrderId = params.get('orderId') || ''
      const nextParams = new URLSearchParams()
      if (nextOrderId) nextParams.set('orderId', nextOrderId)
      navigate(`${user?.isSeller ? '/seller/orders' : '/orders'}${nextParams.toString() ? `?${nextParams.toString()}` : ''}`, {
        replace: true,
      })
      return
    }
    ...
    if (location.pathname === '/seller/orders') {
      const params = new URLSearchParams(location.search || '')
      setSelectedLiveOrderId(params.get('orderId') || '')
    ...
    const orderHistoryMatch = matchPath('/order-history/:orderId', location.pathname)
    if (orderHistoryMatch?.params?.orderId) {
      setSelectedOrderId(orderHistoryMatch.params.orderId)
    ...
    setView('dashboard')
  }, [location.pathname, location.search, navigate, user?.isSeller, user?.role])
  ...
  useEffect(() => {
    if (view !== 'dashboard' || !serviceSlides.length) return undefined
  ...
  useEffect(() => {
    ...
    const syncOrders = (nextBuyerOrders, nextSellerOrders, announceUpdates = false) => {
      ...
      mergedById.forEach((order, orderId) => {
        const nextSnapshot = getOrderActivitySnapshot(order)
        nextActivity.set(orderId, nextSnapshot)
        ...
        const eventKey = getOrderEventKey(previous, nextSnapshot)
        if (!eventKey) return
        ...
        if (recentMutation) {
          ...
          } else if (recentMutation.eventKey === eventKey) {
            recentOrderMutationsRef.current.delete(orderId)
            return
          }
        }
        const notification = getOrderNotificationContent({ order, eventKey, isBuyer, isSeller })
        ...
        queueNotification({
          ...
          dedupeKey: `order:${eventKey}:${orderId}:${nextSnapshot.updatedAt}`,
        })
      })
  ...
  const normalizeThread = useCallback(
    (thread) => {
      ...
      return {
        ...
        sellerAvatar:
          thread.sellerAvatar ||
          sellerParticipant?.avatarUrl ||
          sellerParticipant?.avatar ||
          '',
        buyerAvatar:
          thread.buyerAvatar ||
          buyerParticipant?.avatarUrl ||
          buyerParticipant?.avatar ||
          '',
        ...
        unreadCount:
          ...
          messages.filter(
            (msg) =>
              ...
              !(msg.readBy || []).some((id) => id === (user?._id || user?.id)),
          ).length,
        typingStatuses: (thread.typingStatuses || []).map((entry) => entry.user?.toString?.() || entry.user),
        otherTypingStatuses: (thread.typingStatuses || [])
          .map((entry) => entry.user?.toString?.() || entry.user)
          .filter((id) => id && id !== (user?._id || user?.id)),
      }
    },
    [user],
  )
  ...
  useEffect(() => {
    ...
    const syncThreads = (normalizedThreads, announceUpdates = false) => {
      ...
      const mergedThreads = normalizedThreads.map((thread) => mergeThreadWithOptimisticMessages(thread))
      seenThreadActivityRef.current = nextActivity
      setChatThreads(mergedThreads)
      const total = mergedThreads.reduce((sum, thread) => sum + (thread.unreadCount || 0), 0)
  ...
  }, [authToken, mergeThreadWithOptimisticMessages, normalizeThread, queueNotification, user])
    ...
    authedFetch(`/api/chats/${selectedThreadId}`)
      .then((data) => {
        ...
        setChatThreads((prev) => {
          ...
          const nextThreads = [mergeThreadWithOptimisticMessages(normalized), ...others]
  ...
  }, [authToken, mergeThreadWithOptimisticMessages, normalizeThread, selectedThreadId, user])

  useEffect(() => {
    if (!user || !authToken || !selectedOrderId) return undefined
    let cancelled = false

    authedFetch(`/api/orders/${selectedOrderId}`)
      .then((data) => {
        if (cancelled || !data?.order) return
        const nextOrder = data.order
        const currentUserId = String(user?._id || user?.id || '')
        const buyerId = getComparableUserId(nextOrder?.buyer)
        const sellerId = getComparableUserId(nextOrder?.seller)

        if (buyerId === currentUserId) {
          setBuyerOrders((prev) => upsertOrderList(prev, nextOrder))
        }
        if (sellerId === currentUserId) {
          setSellerOrders((prev) => upsertOrderList(prev, nextOrder))
        }
      })
      .catch((error) => {
  ...
  }, [authToken, selectedOrderId, user])
  ...
  const handleOpenLiveGigs = useCallback(
    (orderOrId = '') => {
      if (!user) {
      ...
      const nextOrderId =
        typeof orderOrId === 'string'
          ? orderOrId
          : String(orderOrId?._id || orderOrId?.id || '')

      const params = new URLSearchParams()
      if (nextOrderId) params.set('orderId', nextOrderId)
      setSelectedLiveOrderId(nextOrderId)
      const targetPath = user.isSeller ? '/seller/orders' : '/orders'
  ...
  const handleOpenChatFromOrder = async (order) => {
    ...
    await runWithActionCooldown(
      `chat:order:${getOrderIdentity(order)}`,
      async () => {
        try {
  ...
  const handleOpenOrderHistory = useCallback(
    (orderOrId, options = {}) => {
      ...
      const nextPath = `/order-history/${nextOrderId}${params.toString() ? `?${params.toString()}` : ''}`
      navigate(nextPath)
    },
    [navigate],
  )
  ...
  const handleBackFromOrderHistory = useCallback(() => {
    ...
    navigate('/orders')
  }, [location.search, navigate, user?.isSeller])
  ...
  const handleUpdateOrderStatus = async (orderId, status, cancelReason = '') => {
    ...
    await runWithActionCooldown(
      ...
      async () => {
        try {
          ...
          if (data?.order) {
            ...
            const isSeller = data.order.seller?.toString?.() === userId || data.order.seller === userId
            if (isBuyer) {
              setBuyerOrders((prev) => upsertOrderList(prev, data.order))
            }
            if (isSeller) {
              setSellerOrders((prev) => upsertOrderList(prev, data.order))
            }
            ...
            if (data.event === 'accepted') {
              ...
              if (updatedOrderId) {
                setSelectedLiveOrderId(updatedOrderId)
                handleOpenLiveGigs(updatedOrderId)
              }
            } else if (data.event === 'cancellation_requested') {
              queueNotification({
                type: 'order',
                title: 'Cancellation Requested',
                body: `${data.order.gigTitle || 'Your order'} - Waiting for the other party to confirm cancellation.`,
                action: selfAction,
                dedupeKey: `order:self:cancellation-requested:${updatedOrderId}`,
              })
              setMessage('Cancellation request sent. The order stays active until the other party confirms.')
      ...
      MUTATION_ACTION_COOLDOWN_MS,
    )
  }

  const handleRequestOrderCancel = (orderOrId, reason = '') => {
    const orderId = typeof orderOrId === 'string' ? orderOrId : getOrderIdentity(orderOrId)
    const currentUserId = String(user?._id || user?.id || '')
    const otherPartyRequestedCancellation =
      typeof orderOrId === 'object' &&
      orderOrId &&
      ((currentUserId === String(getComparableUserId(orderOrId?.buyer) || '') &&
        orderOrId?.sellerCancellationRequestedAt) ||
        (currentUserId === String(getComparableUserId(orderOrId?.seller) || '') &&
          orderOrId?.buyerCancellationRequestedAt))
    const trimmedReason = String(reason || '').trim()
    if (!trimmedReason && !otherPartyRequestedCancellation) {
      setMessage('Cancellation reason is required.')
      return
    }
    handleUpdateOrderStatus(orderId, 'cancelled', trimmedReason)
  }

  const handleRequestOrderComplete = (orderOrId) => {
    const orderId = typeof orderOrId === 'string' ? orderOrId : getOrderIdentity(orderOrId)
    handleUpdateOrderStatus(orderId, 'complete')
  }

  const handleRequestOrderRejectCompletion = (orderOrId) => {
    const orderId = typeof orderOrId === 'string' ? orderOrId : getOrderIdentity(orderOrId)
    handleUpdateOrderStatus(orderId, 'in_progress')
  }

  const handleRequestOrderAccept = (orderOrId) => {
    const orderId = typeof orderOrId === 'string' ? orderOrId : getOrderIdentity(orderOrId)
    handleUpdateOrderStatus(orderId, 'in_progress')
  }
      ...
      async () => {
        try {
          ...
          if (data?.order) {
            setBuyerOrders((prev) => upsertOrderList(prev, data.order))
            if (data.reused) {
              setMessage(getActiveOrderStatusMessage(data.order.status) || data.message)
              return
            }
            queueNotification({
              type: 'order',
              title: 'Order Placed',
          body: `${gig.title || 'Your order'} • Seller has been notified.`,
              action: { kind: 'buyer-orders' },
              dedupeKey: `order:placed:${getOrderIdentity(data.order)}`,
        })
  ...
  const handleStartGigFromChat = async () => {
    ...
    const existingOrder =
      selectedThreadActiveOrder ||
      buyerOrders.find((order) =>
        isActiveChatOrder(order, {
          gigId: gig.id,
          buyerId: user._id || user.id || '',
          sellerId: selectedThread.sellerUserId || gig.sellerUserId || '',
        }),
      )
    if (existingOrder) {
      setMessage(getActiveOrderStatusMessage(existingOrder.status))
      isStartingGigFromChatRef.current = false
      setIsStartingGigFromChat(false)
      return
    }
  ...
  const handleSendMessage = async (event) => {
    ...
    const composerFileSnapshot = [...composerFiles]
    const optimisticMessageId = `temp-msg-${now}-${Math.random().toString(16).slice(2)}`
    const currentUserId = user?._id || user?.id || ''
    ...
    const optimisticMessage = {
      ...
      text,
      sentAt: now,
      attachments: composerFileSnapshot.map((file) => ({
        id: file.id,
        name: file.name,
        type: file.type || 'file',
      ...
      deliveryStatus: composerFileSnapshot.length > 0 ? 'uploading' : 'sending',
    }

    setComposerText('')
    setComposerFiles([])
    if (threadId && authToken) {
      authedFetch(`/api/chats/${threadId}/typing`, {
        method: 'POST',
        body: { ttlSeconds: 0 },
      }).catch(() => {})
    }
    setOptimisticMessageForThread(threadId, optimisticMessage)
    setPendingChatSendCount((count) => count + 1)
    ...
    const finalizeSentMessage = (targetThreadId, data, uploadedFiles = []) => {
      const sentAt = data?.message?.createdAt ? new Date(data.message.createdAt).getTime() : Date.now()
      const newMessage = data?.message
        ? {
            id: data.message._id || data.message.id || `msg-${sentAt}`,
            senderRole: user?.isSeller ? 'seller' : 'buyer',
            senderId:
            ...
            readBy: (data.message.readBy || []).map((id) =>
      ...
      clearOptimisticMessageForThread(targetThreadId, optimisticMessageId)
      upsertChatThreadMessage(targetThreadId, newMessage)
    ...
    ;(async () => {
      try {
        let uploadedFiles = []
        if (composerFileSnapshot.length > 0) {
          uploadedFiles = await uploadChatFiles(composerFileSnapshot)
          if (uploadedFiles.some((file) => !file.uploadedUrl)) {
            setMessage('Some attachments could not be uploaded. Sending preview only.')
          }
          updateOptimisticMessageForThread(threadId, optimisticMessageId, (message) => ({
            ...message,
        ...
        const payload = {
          ...
          gigId: threadSnapshot.gigId || selectedGig?.id || '',
          gigTitle: threadSnapshot.gigTitle || selectedGig?.title || '',
          sellerId: payloadSellerId,
          buyerId: payloadBuyerId || currentUserId,
          sellerName: threadSnapshot.sellerName || selectedGig?.seller || 'Seller',
          buyerName:
            threadSnapshot.buyerName ||
        ...
        const postMessage = (targetThreadId) =>
          authedFetch(`/api/chats/${targetThreadId}/messages`, {
            method: 'POST',
            body: payload,
          })

        const data = await postMessage(threadId)
        finalizeSentMessage(threadId, data, uploadedFiles)
      } catch (error) {
        const nextMessage = error.message || 'Unable to send message.'
        if (/Thread not found/i.test(nextMessage)) {
          try {
            const gigContext =
              selectedGig ||
              (threadSnapshot?.gigId
                ? {
                    id: threadSnapshot.gigId,
                    title: threadSnapshot.gigTitle,
                    seller: threadSnapshot.sellerName,
                    sellerId: threadSnapshot.sellerUserId,
                    sellerUserId: threadSnapshot.sellerUserId,
                    owner: threadSnapshot.sellerUserId,
                  }
                : null)

            if (gigContext) {
              const ensuredThread = await ensureThreadForGig(gigContext, { forceCreate: true })
              clearOptimisticMessageForThread(threadId, optimisticMessageId)
              setOptimisticMessageForThread(ensuredThread.id, optimisticMessage)
              setSelectedThreadId(ensuredThread.id)
              navigate(`/chats/${ensuredThread.id}`)

              const retryData = await authedFetch(`/api/chats/${ensuredThread.id}/messages`, {
                method: 'POST',
                body: {
                  text,
                  files: optimisticMessage.attachments.map((file) => ({
                    name: file.name,
                    type: file.type || 'file',
                    size: composerFileSnapshot.find((entry) => entry.id === file.id)?.size || 0,
                    url: file.url || file.previewUrl || '',
                  })),
                  gigId: ensuredThread.gigId || threadSnapshot.gigId || '',
                  gigTitle: ensuredThread.gigTitle || threadSnapshot.gigTitle || '',
                  sellerId: ensuredThread.sellerUserId || threadSnapshot.sellerUserId || '',
                  buyerId: ensuredThread.buyerUserId || threadSnapshot.buyerUserId || currentUserId,
                  sellerName: ensuredThread.sellerName || threadSnapshot.sellerName || 'Seller',
                  buyerName: ensuredThread.buyerName || threadSnapshot.buyerName || user.name || 'Buyer',
                  participantIds: [ensuredThread.sellerUserId, ensuredThread.buyerUserId].filter(Boolean),
                },
              })
              finalizeSentMessage(ensuredThread.id, retryData)
              return
            }
          } catch (recreateError) {
            updateOptimisticMessageForThread(threadId, optimisticMessageId, (message) => ({
            ...
            setMessage(recreateError.message || nextMessage)
            return
          }
        ...
        setPendingChatSendCount((count) => Math.max(0, count - 1))
      }
    })()
  }
  ...
  const handleOpenChatFromGig = async (gig) => {
    ...
    await runWithActionCooldown(
      `chat:gig:${gigId || gig.sellerId || gig.owner || 'unknown'}`,
      async () => {
        if (gigId) {
  ...
  const handleGoogleSignIn = async (credential) => {
    if (isAuthLoading) return
    const token = (credential || '').toString().trim()
    if (!token) {
      const errorMessage = 'Google Sign-In did not return a valid credential.'
      setFormErrors((prev) => ({ ...prev, login: errorMessage, signup: errorMessage }))
      setMessage(errorMessage)
      return
    }

    setFormErrors((prev) => ({ ...prev, login: '', signup: '' }))
    await runWithActionCooldown(
      'auth:google-signin',
      async () => {
        try {
          ...
          setMessage(data?.isNewUser ? 'Account created with Google.' : 'Welcome back.')
          navigate('/')
        } catch (error) {
          const errorMessage = error.message || 'Unable to sign in with Google.'
          setFormErrors((prev) => ({ ...prev, login: errorMessage, signup: errorMessage }))
          setMessage(errorMessage)
        } finally {
          setIsAuthLoading(false)
  ...
  const handleCreateGig = async (event) => {
    ...
    await runWithActionCooldown(
      editingGigId ? `gig:update:${editingGigId}` : 'gig:create',
      async () => {
        try {
      ...
      async () => {
        try {
          ...
          setGigReviews((prev) => ({
            ...
            [selectedGig.id]: [
              newReview,
              ...(prev[selectedGig.id] || []).filter(
                (review) =>
                  String(review.id || '') !== String(newReview.id || '') &&
                  !(
                    String(review.buyerId || '') === String(newReview.buyerId || '') &&
                    String(review.gigId || '') === String(newReview.gigId || '')
                  ),
              ),
            ],
          }))
          if (selectedGig.sellerId) {
            setSellerReviews((prev) => ({
              ...
              [selectedGig.sellerId]: [
                newReview,
                ...(prev[selectedGig.sellerId] || []).filter(
                  (review) =>
                    String(review.id || '') !== String(newReview.id || '') &&
                    !(
                      String(review.buyerId || '') === String(newReview.buyerId || '') &&
    ...
    >
      ...
      <div
        className={`mx-auto flex w-full flex-col gap-6 ${
          view === 'chat'
            ? 'max-w-none flex-1 min-h-0 overflow-hidden px-0 py-0'
            : 'max-w-6xl px-4 py-6 sm:px-6 sm:py-8 lg:px-8'
        }`}
      >
        <TimedAlert
        ...
        {view === 'chat' && (
          <div className="flex-1 min-h-0 overflow-hidden">
            <ChatView
              ...
              activeOrderStatus={
                selectedThreadActiveOrder ? normalizeOrderStatus(selectedThreadActiveOrder.status) : ''
              }
              activeOrderId={getOrderIdentity(selectedThreadLatestOrder)}
              onOpenOrder={() => {
                const nextOrder = selectedThreadActiveOrder || selectedThreadLatestOrder
                const nextOrderId = getOrderIdentity(nextOrder)
                if (!nextOrderId) return
                if (selectedThreadActiveOrder) {
                  handleOpenLiveGigs(nextOrderId)
                  return
                }
                handleOpenOrderHistory(nextOrderId, {
                  from: 'chat',
              ...
              onTyping={(value = '') => {
                ...
                authedFetch(`/api/chats/${selectedThread.id}/typing`, {
                  method: 'POST',
                  body: { ttlSeconds: String(value || '').trim() ? 5 : 0 },
                }).catch(() => {})
              }}
              user={user}
            />
        ...
        {view === 'seller-orders' && (
          <SellerOrdersView
            ...
            buyerOrders={buyerOrders}
            sellerOrders={sellerOrders}
            preselectedOrderId={selectedLiveOrderId}
            formatter={formatter}
        ...
        {view === 'order-history' && (
          <OrderHistoryView
            user={user}
            order={selectedOrder}
            isLoading={Boolean(selectedOrderId && !selectedOrder && authToken)}
            formatter={formatter}
            onBack={handleBackFromOrderHistory}
            onOpenGig={handleOpenGigDetail}
            onOpenChat={handleOpenChatFromOrder}
            onAccept={handleRequestOrderAccept}
            onCancel={handleRequestOrderCancel}
            onComplete={handleRequestOrderComplete}
            onRejectCompletion={handleRequestOrderRejectCompletion}
          />
        )}
