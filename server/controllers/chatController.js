...
const normalizeFiles = (raw) => {
  ...
  if (typeof raw === 'string') {
    try {
      ...
      try {
        // Last resort: allow JS-like array/object strings
...
export const getThread = asyncHandler(async (req, res) => {
  ...
  if (!ensureParticipant(thread, req.user.id)) {
    // Heal participant list to cover stale/stub threads
...
export const createThread = asyncHandler(async (req, res) => {
  ...
  // Prevent duplicates per gig + participants
  ...
  if (existing) {
    // ensure gig metadata is present
...
export const addMessage = asyncHandler(async (req, res) => {
  ...
  // Fallback: try to locate or create by gigId + participants when the id is stale
  ...
  if (!ensureParticipant(thread, req.user.id)) {
    // Heal participant list by adding the current user when we can resolve both sides
  ...
  // Sanitize any existing message files that may have been stored as strings
