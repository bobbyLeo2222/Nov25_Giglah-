...
const removeTypingStatusForUser = (thread, userId) => {
  thread.typingStatuses = (thread.typingStatuses || []).filter(
...
const hydrateThread = (thread, currentUserId) => {
  ...
  const unreadCount = messages.reduce(
    (count, msg) => {
      ...
      if (senderId === currentUserId?.toString()) return count
      const hasRead = msg.readBy?.some((id) => id.toString() === currentUserId)
...
export const addMessage = asyncHandler(async (req, res) => {
  ...
  pruneTypingStatuses(thread)
  removeTypingStatusForUser(thread, req.user.id)
...
export const setTyping = asyncHandler(async (req, res) => {
  ...
  if (!Number.isFinite(ttlSeconds) || ttlSeconds <= 0) {
    removeTypingStatusForUser(thread, req.user.id)
    await thread.save()
    return res.json({ message: 'Typing cleared' })
  }
